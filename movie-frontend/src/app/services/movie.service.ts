...
export class MovieService {
  ...
  addMovie(movieData: NewMovie, imageFile?: File): Observable<Movie> {
    const formData = new FormData();
    formData.append('title', movieData.title);
    formData.append('rating', String(movieData.rating));
  
    if (imageFile) {
      formData.append('image', imageFile, imageFile.name);
    }
  
    return this.http.post<Movie>(`${this.apiUrl}/`, formData).pipe(
      tap(() => this.loadInitialData()) // Reload data after adding
    );
  }
  

  updateMovie(movieId: number, movieData: NewMovie, imageFile?: File): Observable<Movie> {
    const formData = new FormData();
    formData.append('title', movieData.title);
    formData.append('rating', String(movieData.rating));
    ...
    return this.http.put<Movie>(`${this.apiUrl}/${movieId}/`, formData).pipe(
      tap(() => {
        this.getMovies().subscribe(movies => {
          this.moviesSubject.next(movies);
        });
